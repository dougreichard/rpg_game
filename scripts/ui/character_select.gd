...
const CHAR_ROLES: Dictionary = {
	"Quinn": "Mechanic & Translator\nBooming laugh stuns nearby enemies",
	"Erin":  "Infiltrator\nTalks past guards, resists detection",
	"Evan":  "Strongman\nCommands animal companions in combat",
	"Ben":   "Bard\nRhythm attacks and perfect pitch",
	"Ethan": "Hacker\nCracks panels, doors, and digital enemies",
}
...
func _build_ui() -> void:
	...
	location_lbl.add_theme_font_size_override("font_size", 20)
	...
	title.size = Vector2(1280.0, 54.0)
	...
	var sub := Label.new()
	sub.text = "The active lead fights first — swap freely with  Tab  at any time"
	...
	hint_nav.add_theme_font_size_override("font_size", 16)
	...
	_press_label.add_theme_font_size_override("font_size", 28)
...
func _confirm() -> void:
	...
	get_tree().change_scene_to_file(GameManager.pending_level)
...
func _draw() -> void:
	...
	for i in _duo.size():
		...
		# Small colored square inside the chip as visual anchor
		var sq := 24.0
		draw_rect(
		...
		# Active/standby badge below chip
		var badge_y: float = CARD_Y + CHIP_H + 10.0
		...
		draw_string(font, Vector2(cx + CARD_W * 0.5, badge_y + 14.0),
				badge_text, HORIZONTAL_ALIGNMENT_CENTER, CARD_W - 10.0, 14, badge_col)

		# Character name
		draw_string(font, Vector2(cx + CARD_W * 0.5, badge_y + 46.0),
				char_name, HORIZONTAL_ALIGNMENT_CENTER, CARD_W - 10.0, 22,
				Color(0.92, 0.92, 0.92) if is_active else Color(0.55, 0.55, 0.62))

		# Special label
		var special_text: String = CHAR_SPECIALS.get(char_name, "")
		draw_string(font, Vector2(cx + CARD_W * 0.5, badge_y + 78.0),
				special_text, HORIZONTAL_ALIGNMENT_CENTER, CARD_W - 10.0, 14,
		...
		for li in lines.size():
			...
			draw_string(font,
					Vector2(cx + CARD_W * 0.5, badge_y + 108.0 + float(li) * 22.0),
					lines[li], HORIZONTAL_ALIGNMENT_CENTER, CARD_W - 16.0, 13, role_col)

	# Arrow hints between the cards
	var mid_x: float = 1280.0 * 0.5
	var arrow_y: float = CARD_Y + CARD_H * 0.5
	var font: Font = ThemeDB.fallback_font
	draw_string(font, Vector2(mid_x - 16.0, arrow_y + 8.0),
			"◄  ►", HORIZONTAL_ALIGNMENT_CENTER, 32.0, 20, Color(0.6, 0.6, 0.7))
