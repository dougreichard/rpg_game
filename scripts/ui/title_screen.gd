...
func _ready() -> void:
	...
	_menu_cursor = IDX_CONTINUE if _has_save else IDX_NEW
	_build_background()
	_build_main_ui()
...
func _is_enabled(idx: int) -> bool:
	if idx == IDX_CONTINUE or idx == IDX_LOAD:
		return _has_save
	return true
...
func _start_new_game() -> void:
	GameManager.completed_locations = []
	GameManager.unlocked_characters = ["quinn", "erin"]
	GameManager.inventories = {}
	GameManager.level_progress = {}
