...
const CONSOLE_POS := Vector2(560.0, 180.0)
...
func _spawn() -> void:
	...
	_add(RUNNER_SCENE, Vector2(480.0, 120.0))
	_spawned = true
...
func _process(_delta: float) -> void:
	...
	if _cleared and Input.is_action_just_pressed("ui_accept"):
		GameManager.complete_location("recording_studio")
...
func _update_hint() -> void:
	if _cleared:
		...
		hint_label.text = "Someone scrambled the studio — clear them out!"
	elif not _console_tuned:
		hint_label.text = "Ben: tune the soundboard by ear  [ approach it, press G ]"
	else:
		hint_label.text = ""
