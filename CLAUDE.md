...
- **Floor 1:** Lobby (Bellows + organ console; combat-free) · Storeroom (grunts + hidden Erin + the warped plank) · **Workshop** (table saw + tuning bench + the bellows blower engine) · **Materials/Timber Store** annex (lumber stock + Erin's supplier cabinet) · stair alcove. **Floor 2:** Pipe Loft (out-of-tune pipe + gear blank; runners) + secret spare-gear nook. *(Rooms enlarged +~40% with longer corridors — the indoor adaptation of the outdoor-recipe "bigger spaces" pass.)*
- **Key puzzle(s):** A **gather → mill → assemble** crafting chain (uses the reusable `WorkStation3D` kit). **First, Quinn restarts the bellows blower engine to power the workshop tools** (the saw + bench are dead until then — a mechanical prerequisite, like Carnival's power box). Quinn collects three raw materials across both floors — `rough_plank` (Storeroom), `rough_organ_pipe` + `gear_blank` (Pipe Loft) — and processes them at the powered Workshop tools: **table saw** (`rough_plank`→`windchest_board`, `rough_organ_pipe`→`cut_organ_pipe`), **tuning bench** (`cut_organ_pipe`→`brass_organ_pipe`, `gear_blank`→`trued_gear`). The pipe needs **both** tools (saw *then* tune). Erin fast-talks the `tuning_key` out of Bellows, **unlocking the organ console** (ASSEMBLY) so Quinn fits the three finished parts. **Optional:** Erin fast-talks the **supplier cabinet** in the Timber Store → `bent_spoon` (lore).
- **Enemy types:** Grunts (Storeroom) + Runners (Pipe Loft)
- **3D build:** reference for the multi-room kit — rooms joined by `corridor()`s with solid-colour corner posts; **per-room thematic surfaces** (marble lobby / wood workshop / concrete service corridors / tile-stone loft) via `set_theme`. **Hero + dressing props are Prop-Farm** (organ, table_saw, tuning_bench, bellows_engine, pipe_rack, lumber_stack, wall tool_board) + Synty (desk/shelf/barrels); wall tool-boards are the indoor "boundary" detail. **Secret-nook fix:** the loft has a real `"n"` doorway the removable panel fills — the spare-gear nook is reachable.
- **Level progress flags:** `enemies_cleared` / `organ_repaired` / `secret_revealed` / `manager_met` / `tuning_key_given` / `erin_recruited` / `power_on` / `cabinet_done` / `plank_taken` / `pipe_taken` / `gear_taken` / `organ_part_<id>` / `gear_bonus_open`
...
- **3D build:** expanded multi-room — combat-free OFFICE (Viktor + exit) → big DOCK YARD 24×20 (combat + crane chain) → optional STOREROOM (east, `boiler_key`), behind 4m corridors. Chain: Evan **container** → Quinn **dock-power** → run the **crane** (needs `crane_crank_handle` from the yard) → lifts the manifest crate (`doug_crate_tag`). The **`pocket_lantern`** is now in the manifest locker (Evan force / Quinn dial). **Hero props are Prop-Farm** — the gantry crane (`dock_crane_*`), a real shipping container for Evan's container (sinks when shoved) + container stacks, a forklift, mooring bollards along the water, and life buoys; manifest crate reuses `deck_crate`. Concrete/dirt surfaces + a harbour water strip, rust trim.
...
- **3D build:** concrete/dirt/stone surfaces per depth (kept dim — lantern-lit). Opening the vault grants `doug_flashlight`. **Hero props are Prop-Farm** (`pump_machine` in the pump room, a `vault_door` bank-safe in the vault room, `valve_wheel` for the drain + Evan's wheel, `pipe_cluster` wall pipes across the depths); the flat blast door stays primitive for its open-tween.
