#!/usr/bin/env python3
"""
Ben sprite sheet — ComfyUI img2img + IP-Adapter identity conditioning.

Same "Option B" pipeline as gen_quinn_comfy_ipadapter.py / gen_erin_comfy_ipadapter.py:

Pipeline:
  STEP 1 — generate ONE reference image of Ben via txt2img (SDXL + LoRA,
           fixed REFERENCE_SEED). This is the "ground truth" Ben.
           (Already generated by generators/gen_character_reference.py ->
           assets/art/sprites/ben_reference.png — reused here as a cache.)
...
Usage:
  ...
  python3 generators/gen_ben_comfy_ipadapter.py                # full run
  python3 generators/gen_ben_comfy_ipadapter.py --ref-only     # just make/preview the reference image
  python3 generators/gen_ben_comfy_ipadapter.py --rows idle,walk_down --max-frames-per-row 2
"""
...
PIL_PATH   = os.path.join(ROOT, "assets", "art", "sprites", "ben.png")
OUT_PATH   = PIL_PATH   # overwrite with the ComfyUI-enhanced version
PLACEHOLDER_PATH = os.path.join(ROOT, "assets", "art", "sprites", "ben_placeholder.png")
REF_PATH   = os.path.join(ROOT, "assets", "art", "sprites", "ben_reference.png")
DEBUG_PATH = os.path.join(ROOT, "assets", "art", "sprites", "ben_debug_ip")
...
CHARACTER_SEED = 6184        # fixed for all per-frame img2img passes
REFERENCE_SEED = 9303        # fixed seed for the one identity-reference image (matches gen_character_reference.py)
...
BASE_NEG = (
    ...
    "silhouette, panel, frame, border, vignette, circle, glow, halo, spotlight"
)
REFERENCE_POSE = "standing still front view, neutral pose, arms relaxed at sides"

FRAMES = [
    # (row_name, pose_hint, n_frames) — order/counts must match gen_ben.py's ROWS
    ("idle",          "standing still front view holding keytar across chest, arms relaxed", 6),
    ("walk_down",     "walking toward viewer front view mid stride, keytar across chest",  8),
    ("walk_up",       "walking away from viewer back view mid stride",                     8),
    ("walk_right",    "walking right side profile mid stride, keytar visible",             8),
    ...
    ("attack",        "swinging the keytar in a wide sweeping strike, side profile",       6),
    ("special",       "listening intently, hand cupped to ear, eyes closed, concentrating", 8),
    ("perfect_pitch", "intense listening then eureka realization, eyes wide, finger raised", 6),
    ("talk",          "talking gesturing hands front view slight lean, theatrical",        6),
    ("talk_closeup",  "head and shoulders portrait front view talking expressively",       8),
    ("hurt",          "recoiling backward from impact grimace",                            4),
    ("down",          "fallen on ground face-down knocked out",                           10),
    ("revive",        "rising from floor one knee up",                                     8),
    ("dash",          "low lunge dash to the right",                                       5),
    ("perform",       "theatrical crowd-addressing performance pose, arms wide, playing keytar", 8),
]
...
def make_reference_workflow(prompt: str, neg: str, seed: int) -> dict:
    ...
    return {
        ...
        "8": {"class_type": "SaveImage",
              "inputs": {"images": ["7", 0], "filename_prefix": "hb_ben_ref_"}},
    }
...
def ensure_placeholder_sprite() -> Image.Image:
    ...
    if not os.path.exists(PLACEHOLDER_PATH):
        print("Placeholder baseline not found — generating via gen_ben.py...")
        gen = os.path.join(ROOT, "generators", "gen_ben.py")
...
def generate_reference(regenerate: bool) -> Image.Image:
    """Generate (or load cached) the single Ben identity-reference image."""
    ...
    print("Generating Ben identity-reference image...", end=" ", flush=True)
...
def main() -> None:
    ...
    ref_name = upload_image(reference, "hb_ben_reference.png")
    ...
    for row, (name, pose, n_frames) in enumerate(FRAMES):
        ...
        for col in range(cols):
            ...
            # Upload to ComfyUI
            init_name = upload_image(init_img, f"hb_ben_ip_r{row:02d}_c{col:02d}.png")
            ...
            wf  = make_img2img_ipadapter_workflow(prompt, BASE_NEG, CHARACTER_SEED,
                                                   ...
                                                   f"hb_ben_ip_r{row:02d}_c{col:02d}_{RUN_ID}_")
            pid = queue_prompt(wf)
