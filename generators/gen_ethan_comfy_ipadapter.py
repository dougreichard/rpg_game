#!/usr/bin/env python3
"""
Ethan sprite sheet — ComfyUI img2img + IP-Adapter identity conditioning.

Same "Option B" pipeline as gen_quinn_comfy_ipadapter.py / gen_erin_comfy_ipadapter.py:

Pipeline:
  STEP 1 — generate ONE reference image of Ethan via txt2img (SDXL + LoRA,
           fixed REFERENCE_SEED). This is the "ground truth" Ethan.
           (Already generated by generators/gen_character_reference.py ->
           assets/art/sprites/ethan_reference.png — reused here as a cache.)
...
Usage:
  ...
  python3 generators/gen_ethan_comfy_ipadapter.py                # full run
  python3 generators/gen_ethan_comfy_ipadapter.py --ref-only     # just make/preview the reference image
  python3 generators/gen_ethan_comfy_ipadapter.py --rows idle,walk_down --max-frames-per-row 2
"""
...
PIL_PATH   = os.path.join(ROOT, "assets", "art", "sprites", "ethan.png")
OUT_PATH   = PIL_PATH   # overwrite with the ComfyUI-enhanced version
PLACEHOLDER_PATH = os.path.join(ROOT, "assets", "art", "sprites", "ethan_placeholder.png")
REF_PATH   = os.path.join(ROOT, "assets", "art", "sprites", "ethan_reference.png")
DEBUG_PATH = os.path.join(ROOT, "assets", "art", "sprites", "ethan_debug_ip")
...
CHARACTER_SEED = 7350        # fixed for all per-frame img2img passes
REFERENCE_SEED = 9401        # fixed seed for the one identity-reference image (matches gen_character_reference.py)
...
FRAMES = [
    # (row_name, pose_hint, n_frames) — order/counts must match gen_ethan.py's ROWS
    ("idle",          "standing still front view holding small cyan hacking device, arms relaxed", 6),
    ("walk_down",     "walking toward viewer front view mid stride, holding device",       8),
    ...
    ("attack",        "quick jab with glowing cyan device, electric burst, side profile",  6),
    ("special",       "leaning in, scanning with device raised toward a panel, focused expression", 8),
    ("panel_interact","crouching with device extended toward a control panel, focused",    6),
    ("talk",          "talking gesturing with device in hand front view slight lean",      6),
    ("talk_closeup",  "head and shoulders portrait front view talking, focused expression", 8),
    ...
    ("lizard_summon", "crouching and extending device upward, summoning gesture",          6),
]
...
def make_reference_workflow(prompt: str, neg: str, seed: int) -> dict:
    ...
    return {
        ...
        "8": {"class_type": "SaveImage",
              "inputs": {"images": ["7", 0], "filename_prefix": "hb_ethan_ref_"}},
    }
...
def ensure_placeholder_sprite() -> Image.Image:
    ...
    if not os.path.exists(PLACEHOLDER_PATH):
        print("Placeholder baseline not found — generating via gen_ethan.py...")
        gen = os.path.join(ROOT, "generators", "gen_ethan.py")
...
def generate_reference(regenerate: bool) -> Image.Image:
    """Generate (or load cached) the single Ethan identity-reference image."""
    ...
    print("Generating Ethan identity-reference image...", end=" ", flush=True)
...
def main() -> None:
    ...
    ref_name = upload_image(reference, "hb_ethan_reference.png")
    ...
    for row, (name, pose, n_frames) in enumerate(FRAMES):
        ...
        for col in range(cols):
            ...
            # Upload to ComfyUI
            init_name = upload_image(init_img, f"hb_ethan_ip_r{row:02d}_c{col:02d}.png")
            ...
            wf  = make_img2img_ipadapter_workflow(prompt, BASE_NEG, CHARACTER_SEED,
                                                   ...
                                                   f"hb_ethan_ip_r{row:02d}_c{col:02d}_{RUN_ID}_")
            pid = queue_prompt(wf)
