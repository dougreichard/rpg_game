#!/usr/bin/env python3
"""
Evan sprite sheet — ComfyUI img2img + IP-Adapter identity conditioning.

Same "Option B" pipeline as gen_quinn_comfy_ipadapter.py / gen_erin_comfy_ipadapter.py:

Pipeline:
  STEP 1 — generate ONE reference image of Evan via txt2img (SDXL + LoRA,
           fixed REFERENCE_SEED). This is the "ground truth" Evan.
           (Already generated by generators/gen_character_reference.py ->
           assets/art/sprites/evan_reference.png — reused here as a cache.)
...
Usage:
  ...
  python3 generators/gen_evan_comfy_ipadapter.py                # full run
  python3 generators/gen_evan_comfy_ipadapter.py --ref-only     # just make/preview the reference image
  python3 generators/gen_evan_comfy_ipadapter.py --rows idle,walk_down --max-frames-per-row 2
"""
...
PIL_PATH   = os.path.join(ROOT, "assets", "art", "sprites", "evan.png")
OUT_PATH   = PIL_PATH   # overwrite with the ComfyUI-enhanced version
PLACEHOLDER_PATH = os.path.join(ROOT, "assets", "art", "sprites", "evan_placeholder.png")
REF_PATH   = os.path.join(ROOT, "assets", "art", "sprites", "evan_reference.png")
DEBUG_PATH = os.path.join(ROOT, "assets", "art", "sprites", "evan_debug_ip")
...
CHARACTER_SEED = 4127        # fixed for all per-frame img2img passes
REFERENCE_SEED = 9201        # fixed seed for the one identity-reference image (matches gen_character_reference.py)
...
FRAMES = [
    # (row_name, pose_hint, n_frames) — order/counts must match gen_evan.py's ROWS
    ...
    ("attack",       "throwing a powerful straight punch side profile fist extended forward follow-through", 6),
    ("special",      "two-finger whistle then pointing commanding gesture, both arms raised", 8),
    ("lift",         "crouching to grab a heavy object then heaving it overhead with both arms straining", 8),
    ...
    ("brace",        "wide stance both arms spread bracing against something",         6),
]
...
def make_reference_workflow(prompt: str, neg: str, seed: int) -> dict:
    ...
    return {
        ...
        "8": {"class_type": "SaveImage",
              "inputs": {"images": ["7", 0], "filename_prefix": "hb_evan_ref_"}},
    }
...
def ensure_placeholder_sprite() -> Image.Image:
    ...
    if not os.path.exists(PLACEHOLDER_PATH):
        print("Placeholder baseline not found — generating via gen_evan.py...")
        gen = os.path.join(ROOT, "generators", "gen_evan.py")
...
def generate_reference(regenerate: bool) -> Image.Image:
    """Generate (or load cached) the single Evan identity-reference image."""
    ...
    print("Generating Evan identity-reference image...", end=" ", flush=True)
...
def main() -> None:
    ...
    ref_name = upload_image(reference, "hb_evan_reference.png")
    ...
    for row, (name, pose, n_frames) in enumerate(FRAMES):
        ...
        for col in range(cols):
            ...
            # Upload to ComfyUI
            init_name = upload_image(init_img, f"hb_evan_ip_r{row:02d}_c{col:02d}.png")
            ...
            wf  = make_img2img_ipadapter_workflow(prompt, BASE_NEG, CHARACTER_SEED,
                                                   ...
                                                   f"hb_evan_ip_r{row:02d}_c{col:02d}_{RUN_ID}_")
            pid = queue_prompt(wf)
