#!/usr/bin/env python3
"""
Quinn sprite sheet — ComfyUI img2img + IP-Adapter identity conditioning.

"Option B" follow-up to gen_quinn_comfy.py (Option A: fixed-seed img2img
alone produced inconsistent character identity across frames — different
hat shapes, coat colors, faces from frame to frame).

Pipeline:
  STEP 1 — generate ONE reference image of Quinn via txt2img (SDXL + LoRA,
           fixed REFERENCE_SEED). This is the "ground truth" Quinn.
  STEP 2 — for every animation frame:
           PIL pose frame (32x32) -> composite on white, upscale to 1024
             -> img2img init image (structure/pose guidance, as in Option A)
...
Usage:
  ...
  python3 generators/gen_quinn_comfy_ipadapter.py                # full run
  python3 generators/gen_quinn_comfy_ipadapter.py --ref-only     # just make/preview the reference image
  python3 generators/gen_quinn_comfy_ipadapter.py --rows idle,walk_down --max-frames-per-row 2
"""
...
PLACEHOLDER_PATH = os.path.join(ROOT, "assets", "art", "sprites", "quinn_placeholder.png")
REF_PATH   = os.path.join(ROOT, "assets", "art", "sprites", "quinn_reference.png")
DEBUG_PATH = os.path.join(ROOT, "assets", "art", "sprites", "quinn_debug_ip")
...
CHARACTER_SEED = 7142        # fixed for all per-frame img2img passes
REFERENCE_SEED = 9101        # fixed seed for the one identity-reference image
...
BASE_NEG = (
    ...
    "beige background, shadow, colored clothing other than black, "
...
def make_reference_workflow(prompt: str, neg: str, seed: int) -> dict:
    ...
    return {
        ...
        "8": {"class_type": "SaveImage",
              "inputs": {"images": ["7", 0], "filename_prefix": "hb_quinn_ref_"}},
    }
...
def ensure_placeholder_sprite() -> Image.Image:
    ...
    if not os.path.exists(PLACEHOLDER_PATH):
        print("Placeholder baseline not found — generating via gen_quinn.py...")
...
def generate_reference(regenerate: bool) -> Image.Image:
    """Generate (or load cached) the single Quinn identity-reference image."""
    ...
    print("Generating Quinn identity-reference image...", end=" ", flush=True)
...
def main() -> None:
    ...
    ref_name = upload_image(reference, "hb_quinn_reference.png")
    ...
    for row, (name, pose, n_frames) in enumerate(FRAMES):
        ...
        for col in range(cols):
            ...
            # Upload to ComfyUI
            init_name = upload_image(init_img, f"hb_quinn_ip_r{row:02d}_c{col:02d}.png")
            ...
            wf  = make_img2img_ipadapter_workflow(prompt, BASE_NEG, CHARACTER_SEED,
                                                   ...
                                                   f"hb_quinn_ip_r{row:02d}_c{col:02d}_{RUN_ID}_")
            pid = queue_prompt(wf)
